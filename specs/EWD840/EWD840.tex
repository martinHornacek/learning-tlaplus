...
\begin{document}
\tlatex
\@x{}\moduleLeftDash\@xx{ {\MODULE} EWD840}\moduleRightDash\@xx{}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
TLA+ specification of an algorithm for distributed termination
 detection on a ring, due to \ensuremath{Dijkstra}, published as
 \ensuremath{EWD} 840:
 Derivation of a termination detection algorithm for distributed
 computations (with \ensuremath{W.H.J.Feijen} and \ensuremath{A.J.M}. van
 \ensuremath{Gasteren}).
\end{cpar}%
\end{lcom}%
\@x{ {\EXTENDS} Naturals}%
\@pvspace{8.0pt}%
\@x{ {\CONSTANT} N}%
 \@x{ {\ASSUME} NAssumption \.{\defeq} N \.{\in} Nat \.{\,\backslash\,} \{ 0
 \}}%
\@pvspace{8.0pt}%
\@x{ {\VARIABLES} active ,\, color ,\, tpos ,\, tcolor}%
\@pvspace{8.0pt}%
\@x{ Node \.{\defeq} 0 \.{\dotdot} N \.{-} 1}%
\@x{ Color \.{\defeq} \{\@w{white} ,\,\@w{black} \}}%
\@pvspace{8.0pt}%
\@x{ TypeOK \.{\defeq}}%
 \@x{\@s{16.4} \.{\land}\@s{9.74} active \.{\in} [ Node \.{\rightarrow}
 {\BOOLEAN} ]}%
\@y{\@s{0}%
 status of nodes (active or passive)
}%
\@xx{}%
 \@x{\@s{16.4} \.{\land}\@s{9.74} color \.{\in} [ Node \.{\rightarrow} Color
 ]}%
\@y{\@s{0}%
 color of nodes
}%
\@xx{}%
\@x{\@s{16.4} \.{\land}\@s{9.74} tpos \.{\in} Node}%
\@y{\@s{0}%
 token position
}%
\@xx{}%
\@x{\@s{16.4} \.{\land}\@s{9.74} tcolor \.{\in} Color}%
\@y{\@s{0}%
 token color
}%
\@xx{}%
...
\@pvspace{8.0pt}%
\@x{ Init \.{\defeq}}%
\@x{\@s{16.4} \.{\land} active \.{\in} [ Node \.{\rightarrow} {\BOOLEAN} ]}%
\@x{\@s{16.4} \.{\land} color \.{\in} [ Node \.{\rightarrow} Color ]}%
\@x{\@s{16.4} \.{\land} tpos \.{\in} Node}%
\@x{\@s{16.4} \.{\land} tcolor \.{=}\@w{black}}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
Node 0 may initiate a probe when it has the token and when either it is
 black or the token is black. It passes a white token to node
 \ensuremath{N\.{-}1} and
 paints itself white.
\end{cpar}%
\end{lcom}%
\@pvspace{8.0pt}%
\@x{ InitiateProbe \.{\defeq}}%
\@x{\@s{16.4} \.{\land} tpos \.{=} 0}%
 \@x{\@s{16.4} \.{\land} tcolor \.{=}\@w{black} \.{\lor} color [ 0 ]
 \.{=}\@w{black}}%
\@x{\@s{16.4} \.{\land} tpos \.{'} \.{=} N \.{-} 1}%
\@x{\@s{16.4} \.{\land} tcolor \.{'}\@s{0.56} \.{=}\@w{white}}%
\@x{\@s{16.4} \.{\land} active \.{'} \.{=} active}%
 \@x{\@s{16.4} \.{\land} color \.{'} \.{=} [ color {\EXCEPT} {\bang} [ 0 ]
 \.{=}\@w{white} ]}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 A node \ensuremath{i} different from 0 that possesses the token may pass it
 to node
 \ensuremath{i\.{-}1} under the following circumstances\ensuremath{\.{:}
}%
\end{cpar}%
\begin{cpar}{0}{T}{F}{5.0}{0}{}%
\ensuremath{\.{-}} node \ensuremath{i} is inactive or
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
- node \ensuremath{i} is colored black or
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
- the token is black.
\end{cpar}%
...
Note that the last two conditions will result in an inconclusive round,
 since the token will be black. The token will be stained if node
 \ensuremath{i} is
 black, otherwise its color is unchanged. Node \ensuremath{i} will be made
 white.
\end{cpar}%
\end{lcom}%
\@pvspace{8.0pt}%
\@x{ PassToken ( i ) \.{\defeq}}%
\@x{\@s{16.4} \.{\land} tpos \.{=} i}%
 \@x{\@s{16.4} \.{\land} {\lnot} active [ i ] \.{\lor} color [ i ]
 \.{=}\@w{black} \.{\lor} tcolor \.{=}\@w{black}}%
\@x{\@s{16.4} \.{\land} tpos \.{'} \.{=} i \.{-} 1}%
 \@x{\@s{16.4} \.{\land} tcolor \.{=} {\IF} color [ i ] \.{=}\@w{black}
 \.{\THEN}\@w{black} \.{\ELSE} tcolor}%
\@x{\@s{16.4} \.{\land} active \.{'} \.{=} active}%
 \@x{\@s{16.4} \.{\land} color \.{'} \.{=} [ color {\EXCEPT} {\bang} [ i ]
 \.{=}\@w{white} ]}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\end{lcom}%
\@pvspace{8.0pt}%
 \@x{ System \.{\defeq} InitiateProbe \.{\lor} \E\, i \.{\in} Node
 \.{\,\backslash\,} \{ 0 \} \.{:} PassToken ( i )}%
\@x{}\bottombar\@xx{}%
\end{document}
